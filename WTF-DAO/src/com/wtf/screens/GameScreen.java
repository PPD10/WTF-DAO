...
import com.wtf.dao.LevelDao.Properties;
...
public class GameScreen implements Screen {
	...
	public void render(float delta) {
		...
		if (!game.gameOver()) {
			...
			// Sauvegarde éventuelle du score
			checkLevelMaxScoreDB();
	...
	private void initializeMaxScoreDB() {
		// Récupération de l'éventuel niveau dans la BD
		Level level = wtf
				.getDaoSession()
				.getLevelDao()
				.queryBuilder()
				.where(Properties.Name.eq(game.getLevel().getClass().getName()))
				.unique();

		// S'il y en a un, son score maximum est récupéré
		if (level != null)
			game.getMaxScore().setMaxScore(level.getMaxScore());
	}

	private void checkLevelMaxScoreDB() {
		// Récupération de l'éventuel niveau dans la BD
		Level level = wtf
				.getDaoSession()
				.getLevelDao()
		...
		// S'il n'y en a pas un, on l'insère
		if (level == null) {
			level = new Level(null, game.getLevel().getClass().getName(), game
					.getScore().getScore());
			wtf.getDaoSession().getLevelDao().insert(level);
		} else {
			if (level.getMaxScore() < game.getScore().getScore()) {
				level.setMaxScore(game.getScore().getScore());
				wtf.getDaoSession().getLevelDao().update(level);
			}
		}
	}
